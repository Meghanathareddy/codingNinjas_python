class StateMachine:
    ...
    def append_digit(self, digit:int) -> None:
        ...
        if ((self.__result > self.INT_MAX //10) or (self.__result == self.INT_MAX)//10 and digit > self.INT_MAX %10):
            ...
            if self.__sign == 1:
                ...
                self.__sign = 1
            #when the 32-bit int range is exceeed, a dead state is reached.
            self.to_state_qd()
        else:
    ...
    def transition(self, ch:chr) -> None:
        "changed state based on current input character."
        if self.__current_state == self.State["q0"]:
            #Beginning state of the string (or sone whitespaces are skipped).
            if ch == ' ':
